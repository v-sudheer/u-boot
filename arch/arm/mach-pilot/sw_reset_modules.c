...
int Wait_4_Tx_Rx_Stopped(unsigned int mac_base)
{
	...
	while(temp)
	{
		...
		*(tPVU32)(mac_base+CSR0BMR)=CSR0SWR;
		while((( *(tPVU32)(mac_base+CSR0BMR)) &CSR0SWR) && --timeout)
		{
			//nc_printf("waiting for %x s/w reset bit to be cleared\n",mac_base);
		}
...
void sdhc_not_busy(void)
{
	...
	for(i=0; i<3; i++)
	{
		...
		while(((data & 0xf) != 0) && --timeout)
			data = READ_REG32(SDHC_BASE+0x110);
		data = READ_REG32(SDHC_BASE+0x118);
		timeout = 0xffff;

		while(((data & 0xff000000) != 0) && --timeout)
			data = READ_REG32(SDHC_BASE+0x118);
		data = READ_REG32(SDHC_BASE+0x11C);
		timeout = 0xffff;

		while(((data & 0xf0000000) != 0) && --timeout)
			data = READ_REG32(SDHC_BASE+0x11C);
		...
		while (((data & 0x00f00000) != 0x00f00000)&& --timeout)
			data = *(tPVU32)(SDHC_BASE + PRSNTST_REG);
		//  DELAY(1000);
	}
}
...
#ifdef CONFIG_BOMBER_RESET
	/* Set all slots of lmem_13 to initiator 3 in priority register1, which is bomber */
	*(tPVU32)(FGBBASE + 0x54) = 0xdb6db6db;

	/* set slots of lmem13 for LDMA/ARM alternatively in LMEM priority register 2 and 3 */
	*(tPVU32)(FGBBASE + 0x8c) = 0x41041041;
	*(tPVU32)(FGBBASE + 0xac) = 0x08208208;
	modules_reset_rerl |= 1<<18; /* Add bomber to the modules to be reset */
#endif
...
#endif
	usb_dma_disable();
#ifndef CONFIG_BOMBER_PWM_WORKAROUND
