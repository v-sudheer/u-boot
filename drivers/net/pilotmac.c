...
static  int  ast_mac100_open(struct eth_device *);
static void  ast_mac100_close(struct eth_device *);
static void  ast_mac100_reset(struct eth_device *);
static int 	 ast_mac100_alloc_buffers(AST_ETH_PRIV_DATA *lp);
static void  ast_mac100_enable( struct eth_device * );
static int   ast_mac100_recv(struct eth_device *);
static  int  ast_mac100_send_packet(struct eth_device *, void *packet, int length);

static int 	 ast_eth_init(struct eth_device *,bd_t *bd);
...
ast_mac100_recv(struct eth_device *dev)
{
...
#ifdef  ASTMAC_DEBUG
		printf("ASTMAC:(%s):Waiting for full frame %x\n",dev->name,packet_length);
#endif
	}
	...
	for (; start_idx!=lp->rx_idx; start_idx=(start_idx+1)%RXDES_NUM)
	{
		...
		data=(char *)lp->rx_descs[start_idx].Buffer1;
#ifdef  ASTMAC_DEBUG
	...
	net_process_received_packet(data, packet_length);		/* Send to U-Boot layer */
